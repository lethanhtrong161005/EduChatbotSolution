...
function renderPagination() {
    ...
    const totalPages = Page.totalCount > 0
        ? Math.ceil(Page.totalCount / Page.paging.limit)
        : 1;
...
function openCreateModal() {
    ...
    ['create-name', 'create-email'].forEach(id => {
...
async function submitCreate() {
    ...
    try {
        ...
        if (data.success) {
            ...
            performShowToast('success', `Account created. Login credentials sent to ${email}.`);
            await loadUsers();
        } else {
...
async function submitUpdate() {
    ...
    try {
        ...
        if (data.success) {
            ...
            performShowToast('success', 'User updated successfully.');
            await loadUsers();
        } else {
...
async function submitDelete() {
    ...
    try {
        ...
        if (data.success) {
            ...
            performShowToast('success', 'User deleted. Notification email sent.');
            await loadUsers();
        } else {
            closeModal('deleteModal');
            performShowToast('error', data.error ?? 'Failed to delete user.');
        }
    } catch {
        closeModal('deleteModal');
...
async function submitDisable() {
    ...
    try {
        ...
        if (data.success) {
            ...
            performShowToast('success', 'Account disabled. User notified via email.');
            await loadUsers();
        } else {
            closeModal('disableModal');
            performShowToast('error', data.error ?? 'Failed to disable account.');
        }
    } catch {
        closeModal('disableModal');
        performShowToast('error', 'Network error. Please try again.');
...
async function submitReactivate() {
    ...
    try {
        ...
        if (data.success) {
            ...
            performShowToast('success', 'Account reactivated successfully.');
            await loadUsers();
        } else {
            closeModal('reactivateModal');
            performShowToast('error', data.error ?? 'Failed to reactivate account.');
        }
    } catch {
        closeModal('reactivateModal');
        performShowToast('error', 'Network error. Please try again.');
...
function handleImportProgress(update) {
    ...
    if (update.action === "progress-updated" && props.processedRows !== undefined) {
        ...
        if (isCurrentUI) {
            ...
            performShowToast(props.status === 'Completed' ? 'success' : 'warning', `User import finished: ${props.status}`);
            resetImportForm();
        }
        loadUsers();
    }
    ...
    async function (resUpd) {
        switch (resUpd.resourceType) {
            case ResourceType.User:
                performShowToast('info', `${resUpd.resourceName ? "User [" + resUpd.resourceName + "] has" : "Users have"} been updated.`);
                await loadUsers();
                break;
