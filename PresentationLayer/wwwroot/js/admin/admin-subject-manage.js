...
function performShowToast(type, message) {
...
async function submitCreateSubject() {
    ...
    try {
        ...
        if (data.success) {
            ...
            performShowToast('success', 'Subject created successfully.');
...
async function submitUpdateSubject() {
    ...
    try {
        ...
        if (data.success) {
            ...
            performShowToast('success', 'Subject updated successfully.');
            setTimeout(() => location.reload(), 1500);
        } else {
...
async function submitDeleteSubject() {
    ...
    try {
        ...
        if (data.success) {
            ...
            performShowToast('success', 'Subject deleted successfully.');
            setTimeout(() => location.reload(), 1500);
        } else {
            closeModal('deleteSubjectModal');
            performShowToast('error', data.error ?? 'Failed to delete subject.');
        }
    } catch {
        closeModal('deleteSubjectModal');
        performShowToast('error', 'Network error. Please try again.');
    } finally {
...
async function submitCreateChapter() {
    ...
    try {
        ...
        if (data.success) {
            numInput.value = '';
            nameInput.value = '';
            performShowToast('success', 'Chapter added successfully.');
            await loadChapters(subjectId);
        } else {
...
async function saveEditChapter(id) {
    ...
    try {
        ...
        if (data.success) {
            performShowToast('success', 'Chapter updated successfully.');
            await loadChapters(subjectId);
        } else {
...
async function deleteChapter(id) {
    ...
    try {
        ...
        if (data.success) {
            performShowToast('success', 'Chapter deleted successfully.');
            await loadChapters(subjectId);
        } else {
            performShowToast('error', data.error ?? 'Failed to delete chapter.');
        }
    } catch {
        performShowToast('error', 'Network error. Please try again.');
    }
...
async function submitAssignMember() {
    ...
    try {
        ...
        if (data.success) {
            searchInput.value = '';
            searchInput.removeAttribute('data-selected-user-id');
            performShowToast('success', 'Member assigned successfully.');
            await loadMembers(subjectId);
        } else {
...
async function removeMember(userId) {
    ...
    try {
        ...
        if (data.success) {
            performShowToast('success', 'Member removed from subject.');
            await loadMembers(subjectId);
        } else {
            performShowToast('error', data.error ?? 'Failed to remove member.');
        }
    } catch {
        performShowToast('error', 'Network error. Please try again.');
    }
    ...
    async function (resUpd) {
        switch (resUpd.resourceType) {
            case ResourceType.Subject:
                performShowToast('info', `${resUpd.resourceName ? "Subject [" + resUpd.resourceName + "] has" : "Subjects have"} been updated.`);
                await loadSubjects();
                break;
            case ResourceType.Chapter:
                performShowToast('info', `${resUpd.resourceName ? "Chapter [" + resUpd.resourceName + "] has" : "Chapters have"} been updated.`);
            ...
            case ResourceType.Membership:
                performShowToast('info', `${resUpd.resourceName ? "Membership [" + resUpd.resourceName + "] has" : "Memberships have"} been updated.`);
